...
\begin{enumerate}
    \item \textbf{Empirical Irreproducibility:} When evaluated with strict patient/biopsy-level isolation (\texttt{GroupKFold}), multi-class grading on PANDA prostate biopsies collapses from the claimed $88.0\%$ accuracy down to $41.83\%-46.15\%$, and Dice from $99.0\%$ to $28.94\%-44.34\%$.
...
\begin{itemize}
    ...
    \item \textbf{At $\eta=10^{-4}$:} Static weighting yields $43.35\%$ Acc / $37.99\%$ Dice. However, combining $\eta=10^{-4}$ with GradNorm ($\alpha=1.5$) causes severe degradation to $30.89\%$ Acc / $31.59\%$ Dice.
\end{itemize}
This demonstrates that for highly asymmetric multi-task objectives (such as 6-class fine-grained grading coupled with dense segmentation), an aggressive asymmetry parameter ($\alpha=1.5$) heavily suppresses the classification head's gradient updates. For such tasks, dynamic gradient balancing should either utilize milder balancing coefficients ($\alpha \le 0.5$) or decoupled, task-specific projection heads.
