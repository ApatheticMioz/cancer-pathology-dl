...
\begin{enumerate}
    \item \textbf{TCGA-LGG Brain Tumor:} 2-class (lesion vs.\ normal) slice classification and FLAIR-abnormality segmentation on 2D $256 \times 256$ FLAIR MRI slices ($N=3,929$ slices, 110 patients; the 2D release of the TCGA lower-grade glioma collection~\cite{buda2019lgg}).
    \item \textbf{PANDA Prostate Biopsies:} 6-class ISUP Gleason grading (ISUP 0--5) and epithelial gland segmentation ($N=10,616$ tiles~\cite{bulten2022artificial}, with patient-level biopsy isolation via \texttt{GroupKFold}).
    ...
    \item \textbf{PanNuke:} External 19-tissue multi-organ histology benchmark across 5 cell-nuclei classes ($N=7,901$ patches~\cite{gamper2020pannuke}). No multi-task claims were published for this benchmark, so it serves as an unclaimed external control.
\end{enumerate}
