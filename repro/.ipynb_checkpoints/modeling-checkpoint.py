...
class GradNormBalancer(nn.Module):
    ...
    def normalize_(self) -> None:
        with torch.no_grad():
            weights = self.weights().clamp(min=1e-4)
            weights = weights * (2.0 / weights.sum().clamp(min=1e-4))
            self.log_weights.copy_(weights.log())

    def set_initial_losses(self, seg_loss: torch.Tensor, cls_loss: torch.Tensor) -> None:
        with torch.no_grad():
            self.initial_losses.copy_(torch.tensor([float(seg_loss), float(cls_loss)], device=self.initial_losses.device))
        ...
        for batch_idx, (images, masks, labels) in enumerate(loader):
            ...
            with torch.amp.autocast(device_type="cuda", enabled=use_amp):
                ...
                if gradnorm is not None:
                    ...
                    loss = weights[0].detach() * seg_loss + weights[1].detach() * cls_loss
                else:
            ...
            if train:
                ...
                if gradnorm is not None and not bool(gradnorm.has_initial_losses.item()):
                ...
                if gradnorm is not None:
                    ...
                    def _grad_norm(grads: list[torch.Tensor | None]) -> torch.Tensor:
                    ...
                    weights = gradnorm.weights()
                    gradnorm_loss = torch.sum(torch.abs(weights * norms.detach() - target))

                optimizer.zero_grad(set_to_none=True)
                scaler.scale(loss).backward(retain_graph=gradnorm is not None)
...
def train_single_run(
    ...
    while batch_size >= 2:
        ...
        gradnorm = GradNormBalancer(args.lambda_seg, args.lambda_cls, getattr(args, "gradnorm_alpha", 1.5)).to(device)
        optimizer = optim.Adam(list(model.parameters()) + list(gradnorm.parameters()), lr=args.lr)
