...
def _panda_layout(root: Path) -> dict:
    ...
    image_count = 0
    mask_count = 0
    macenko_image_count = 0
    if image_dir:
        image_count = count_files(image_dir, ["*.tif", "*.tiff", "*.png", "*.jpg", "*.jpeg"])
    if mask_dir:
        mask_count = count_files(mask_dir, ["*.tif", "*.tiff", "*.png", "*.jpg", "*.jpeg"])
    if macenko_image_dir.exists():
        macenko_image_count = count_files(macenko_image_dir, ["*.png"])

    return {
        "train_csv": str(train_csv) if train_csv.exists() else None,
        "image_dir": str(image_dir) if image_dir else None,
        "mask_dir": str(mask_dir) if mask_dir else None,
        "macenko_image_dir": str(macenko_image_dir) if macenko_image_dir.exists() else None,
        "image_count": int(image_count),
        "mask_count": int(mask_count),
        "macenko_image_count": int(macenko_image_count),
        "ready": bool(
            train_csv.exists()
            and image_dir
            and mask_dir
            and image_count >= 10000
            and mask_count >= 10000
            and macenko_image_count >= 10000
        ),
    }
...
def _pannuke_layout(root: Path) -> dict:
    ...
    macenko_image_count = count_files(macenko_image_dir, ["*.png"]) if macenko_image_dir.exists() else 0
    row_count = 0
    if index_csv.exists():
        try:
    ...
    return {
        ...
        "ready": bool(
            index_csv.exists()
            and image_count >= 4000
            and mask_count >= 4000
            and row_count >= 4000
            and macenko_image_count >= 4000
        ),
    }
...
PANNUKE_TISSUE_MAP = {name: idx for idx, name in enumerate(PANNUKE_TISSUE_TYPES)}


def _pannuke_archives(root: Path) -> list[Path]:
    return sorted([p for p in root.glob("*.zip") if p.is_file()])


def _build_pannuke_preprocessed(root: Path) -> dict:
    pre_dir = root / "preprocessed"
    image_dir = pre_dir / "images"
    mask_dir = pre_dir / "masks"
    ...
    records = []
    for archive in _pannuke_archives(root):
        with zipfile.ZipFile(archive, "r") as zf:
            image_name = next((n for n in zf.namelist() if n.endswith("images.npy")), None)
            mask_name = next((n for n in zf.namelist() if n.endswith("masks.npy")), None)
            type_name = next((n for n in zf.namelist() if n.endswith("types.npy")), None)
            if not (image_name and mask_name and type_name):
                continue
            ...
            with zf.open(type_name) as f:
                types = np.load(f, allow_pickle=True)

            if images.shape[0] != masks.shape[0] or images.shape[0] != types.shape[0]:
                raise RuntimeError(f"PanNuke archive has mismatched lengths: {archive}")

            for idx in range(images.shape[0]):
                tissue = str(types[idx])
                if tissue not in PANNUKE_TISSUE_MAP:
                    continue

                image = np.clip(images[idx], 0, 255).astype(np.uint8)
                mask = np.argmax(masks[idx], axis=-1).astype(np.uint8)
                image_path = image_dir / f"{archive.stem}_{idx:05d}.png"
                mask_path = mask_dir / f"{archive.stem}_{idx:05d}.png"
                ...
                records.append(
                    {
                        ...
                        "label_int": int(PANNUKE_TISSUE_MAP[tissue]),
                        "group_id": f"{archive.stem}_{idx:05d}",
                    }
                )

    if not records:
        raise RuntimeError("PanNuke preprocessing produced no samples")

    pd.DataFrame.from_records(records).to_csv(pre_dir / "index.csv", index=False)
    return _pannuke_layout(root)
...
def prepare_pannuke(root: Path, force_redownload: bool = False) -> dict:
    ...
    if state["ready"] and not force_redownload:
        print(
            f"  [PANNUKE] ready with images={state['image_count']} masks={state['mask_count']} rows={state['index_rows']} macenko_images={state['macenko_image_count']}"
        )
        return state
    ...
    pre_dir = root / "preprocessed"
    if force_redownload and pre_dir.exists():
        _safe_remove(pre_dir)

    state = _pannuke_layout(root)
    if state["ready"] and not force_redownload:
        print(
            f"  [PANNUKE] ready after local recovery with images={state['image_count']} masks={state['mask_count']} rows={state['index_rows']} macenko_images={state['macenko_image_count']}"
        )
        return state

    print("  [PANNUKE] building preprocessed PNG + mask index from local folds")
    built = _build_pannuke_preprocessed(root)
    if not built["ready"]:
        raise RuntimeError(
            f"PanNuke preprocessing incomplete: images={built['image_count']} masks={built['mask_count']} rows={built['index_rows']} macenko_images={built['macenko_image_count']}"
        )
    return built
...
def prepare_panda(root: Path, force_redownload: bool = False) -> dict:
    ...
    if state["ready"] and not force_redownload:
        print(
            f"  [PANDA] ready with images={state['image_count']} masks={state['mask_count']} macenko_images={state['macenko_image_count']}"
        )
        return state
    ...
    if state["ready"] and not force_redownload:
        print(
            f"  [PANDA] ready after local recovery with images={state['image_count']} masks={state['mask_count']} macenko_images={state['macenko_image_count']}"
        )
        return state
    ...
    if not state["ready"]:
        raise RuntimeError(
            f"PANDA dataset incomplete after redownload/extraction: images={state['image_count']} masks={state['mask_count']} macenko_images={state['macenko_image_count']}"
        )
    return state
